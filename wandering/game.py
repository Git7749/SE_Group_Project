...
class Game:
    ...
    def prep_play_menu(self):
        ...
        for stat in self.stats:
                ...
                play_submenu.add.button(f'{stat.difficulty} min score {min} ', pygame_menu.events.BACK)
                play_submenu.add.button(f'{stat.difficulty} max score {max} ', pygame_menu.events.BACK)
                play_submenu.add.button(f'{stat.difficulty} avg score {avg} ', pygame_menu.events.BACK)
                play_submenu.add.button(f'{stat.difficulty} attempts {attempt}', pygame_menu.events.BACK)    
...
,                       onchange=self.change_rows,
    ...
    def start(self):
        ...
        self.prev_stats()
        self.prep_menu()
        pygame.mixer.music.load('./rituel.mp3')
